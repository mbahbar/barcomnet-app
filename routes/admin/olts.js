...
router.get('/:id/uplinks', requireAdminSession, async (req, res) => {
  try {
    const data = await oltSvc.getZteUplinkPorts(req.params.id);
    res.json(data);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
router.post('/:id/uplinks/config', requireAdminSession, restrictToAdmin, express.urlencoded({ extended: true }), async (req, res) => {
  try {
    const output = await oltSvc.configZteUplinkPort(req.params.id, req.body);
    res.json({ success: true, message: 'Konfigurasi Uplink Port berhasil diterapkan.', output });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

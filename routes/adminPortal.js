...
router.get('/billing/:id/print', requireAdminSession, (req, res) => {
  ...
  res.render('admin/print_invoice', {
    invoice: inv,
    customer,
    company: settings.company_header || 'BARCOMNET',
    settings
  });
});
...
router.get('/api/stats', requireAdmin, async (req, res) => {
  try {
    ...
    const syncStatesMap = customerDevice.getAllBuiltinSyncStatesMap ? customerDevice.getAllBuiltinSyncStatesMap() : null;
    ...
    devices.forEach(d => {
      ...
      const mapped = customerDevice.mapDeviceData(d, d._tags?.[0] || d._id, isPppoeActive, syncStatesMap) || {};
      if (String(mapped.status || '').toLowerCase() === 'online') online++;
      else offline++;
    });
...
router.post('/whatsapp/test-notification', requireAdminSession, async (req, res) => {
  try {
    ...
    const msg =
      ...
      `✅ Jika pesan ini masuk, berarti notifikasi WhatsApp dari Billing BARCOMNET System sudah berfungsi.\n` +
      `📅 Waktu: ${getNowLocal()}`;
...
router.get('/onu-provision', requireAdminSession, restrictToAdmin, (req, res) => {
  ...
  const oltConfig = {
    vendor: getSetting('olt_vendor', 'ZTE'),
    host: getSetting('olt_host', '136.1.1.100'),
    port: getSetting('olt_port', 23),
    username: getSetting('olt_username', 'zte'),
    password: getSetting('olt_password', 'zte')
  };
      ...
      try {
        ...
        if (conn && conn.isTelnet) {
          ...
          await new Promise((resolve, reject) => {
            ...
            const timer = setTimeout(() => { socket.destroy(); reject(new Error('Connection timeout (Port 23)')); }, 10000);
        ...
        req.session._msg = { type: 'success', text: `Koneksi ke OLT ${vendor} (${host}:${port}) berhasil!` };
      } catch (error) {
...
router.post('/onu-provision/scan-unconfigured', requireAdminSession, restrictToAdmin, express.json(), async (req, res) => {
  try {
    ...
    } else if (oltConfig.vendor === 'HSGQ' || oltConfig.vendor === 'Hioso' || oltConfig.vendor === 'Zimmlink' || oltConfig.vendor === 'ZIMMLINK') {
      const { pon } = req.body;
      if (!pon) {
        return res.json({ success: false, error: 'PON interface harus diisi' });
      }

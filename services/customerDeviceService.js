...
function parseFlexibleDate(val) {
  if (!val || val === '-' || val === 'Never' || val === 'Invalid Date') return null;
...
function getAllBuiltinSyncStatesMap() {
  const map = new Map();
  try {
    const rows = db.prepare(
      `SELECT device_id, COUNT(*) AS count, MAX(created_at) AS last_queue_at
       FROM acs_tasks
    ...
    for (const r of rows) {
      if (r && r.device_id) {
        ...
        const lastQueueAt = String(r.last_queue_at || '');
    ...
    logger.debug(`[CustomerDevice] Failed reading all ACS sync states: ${e.message}`);
  }
  return map;
}

function getBuiltinSyncState(deviceId, syncMap = null) {
  const empty = {
  ...
  if (syncMap && syncMap.has(String(deviceId))) return syncMap.get(String(deviceId));
...
function mapDeviceData(device, tag, isPppoeActive = false, syncMap = null) {
  ...
  const syncState = getBuiltinSyncState(device?._id, syncMap);

  let status = 'Unknown';
  const informDate = parseFlexibleDate(device?._lastInform || device?.Events?.Inform);
  if (informDate) {
    const diffMs = Date.now() - informDate.getTime();
...
let listAllDevicesCache = null;
let listAllDevicesCacheTime = 0;
let listAllDevicesInFlight = null;

async function listAllDevices(limit = 999999, acsId = null) {
  const cacheKey = `${limit}_${acsId || 'all'}`;
  const now = Date.now();
  if (listAllDevicesCache && listAllDevicesCache[cacheKey] && (now - listAllDevicesCacheTime) < 5000) {
    return listAllDevicesCache[cacheKey];
  }
  if (listAllDevicesInFlight) {
    return await listAllDevicesInFlight;
  }

  listAllDevicesInFlight = (async () => {
  ...
  try {
    const res = await listAllDevicesInFlight;
    if (!listAllDevicesCache) listAllDevicesCache = {};
    listAllDevicesCache[cacheKey] = res;
    listAllDevicesCacheTime = Date.now();
    return res;
  } finally {
    listAllDevicesInFlight = null;
  }
}

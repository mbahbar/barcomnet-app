...
async function canConnectTcp(host, port, timeoutMs = 500) {
  ...
  return await new Promise((resolve) => {
    ...
    socket.setTimeout(Math.max(150, Number(timeoutMs) || 500));
...
async function getConnection(routerId = null) {
  ...
  if (!selectedPort) {
    ...
    if (!selectedPort) {
      ...
      connectionProbeCache.set(cacheKey, { port: 0, okUntil: 0, failUntil: now + 30000, failMessage });
...
let pppoeSessionsMapCache = null;
let pppoeSessionsMapCacheTime = 0;
let pppoeSessionsMapInFlight = null;

async function getActivePppoeSessionsMap() {
  const now = Date.now();
  if (pppoeSessionsMapCache && (now - pppoeSessionsMapCacheTime) < 10000) {
    return pppoeSessionsMapCache;
  }
  if (pppoeSessionsMapInFlight) {
    return await pppoeSessionsMapInFlight;
  }

  pppoeSessionsMapInFlight = (async () => {
    const routers = getAllRouters().filter(r => r.is_active === 1 || r.is_active === '1' || r.is_active === true);
    const sessionsMap = new Map();
    if (routers.length === 0) return sessionsMap;
  ...
  try {
    const res = await pppoeSessionsMapInFlight;
    pppoeSessionsMapCache = res;
    pppoeSessionsMapCacheTime = Date.now();
    return res;
  } catch (e) {
    if (pppoeSessionsMapCache) return pppoeSessionsMapCache;
    return new Map();
  } finally {
    pppoeSessionsMapInFlight = null;
  }
}

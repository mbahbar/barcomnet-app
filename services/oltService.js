...
const getOnlineValues = (brandKey, profile) => {
  ...
  const isHiosoGpon = (brandKey === 'hioso' || brandKey === 'hsgq' || brandKey === 'zimmlink')
...
const fetchZimmlinkOnusViaTelnetInternal = async (oltConfig, full = false) => {
  return new Promise((resolve) => {
    const host = oltConfig.host || '192.168.100.100';
    const port = oltConfig.telnet_port || 23;
    const user = oltConfig.web_user || 'root';
    const pass = (oltConfig.web_password !== undefined && oltConfig.web_password !== null && oltConfig.web_password !== '') ? oltConfig.web_password : 'P4ssw0rd';
    ...
    const safeResolve = (res) => {
      ...
      resolve(res || Object.values(onusMap));
    };

    const globalTimeout = setTimeout(() => {
      safeResolve(Object.values(onusMap));
    }, full ? 25000 : 12000);
    ...
    client.on('data', (chunk) => {
      ...
      if (state === 0 && buffer.toLowerCase().includes('username:')) {
        state = 1;
        client.write(`${user}\r\n`);
      } else if (state === 1 && buffer.toLowerCase().includes('password:')) {
        state = 2;
        client.write(`${pass}\r\n`);
      } else if (state === 2 && buffer.includes('OLT>')) {
        state = 3;
        client.write('enable\r\n');
      } else if (state === 3 && buffer.includes('OLT#')) {
        state = 4;
        client.write('config\r\n');
      } else if (state === 4 && buffer.includes('OLT(config)#')) {
        state = 5;
        let ponIdx = 1;
        const sendNextPon = () => {
          if (ponIdx <= 4) {
            const currentPon = ponIdx++;
            client.write(`interface epon ${currentPon}\r\n`);
            client.write('show onu-info all\r\n');
            client.write('exit\r\n');
            setTimeout(sendNextPon, 800);
          } else {
            setTimeout(() => {
              const lines = buffer.split('\n');
              for (const line of lines) {
                const match = line.match(/(\d+)\/(\d+)\s+([0-9A-Fa-f:]{17}|[0-9A-Fa-f-]{17}|[0-9A-Fa-f]{12})\s+(Online|Offline)/i);
                if (match) {
                  const pon = match[1];
                  const onuId = match[2];
                  const mac = match[3].replace(/[:-]/g, '').toUpperCase();
                  ...
                  onusMap[key] = {
                    ...
                    sn: mac,
                    mac,
                    name: `ONU ${pon}/${onuId} (${mac.slice(-4)})`,
                    status,
                    rx: 'N/A',
                    tx: 'N/A',
                    rx_power: 'N/A',
              ...
              const allOnus = Object.values(onusMap);
              const onlineOnus = allOnus.filter(o => o.status === 'Online');

              if (!full || onlineOnus.length === 0) {
                client.write('exit\r\n');
                safeResolve(allOnus);
                return;
              }

              const onlineByPon = { 1: [], 2: [], 3: [], 4: [] };
              onlineOnus.forEach(o => {
                if (onlineByPon[o.pon] && onlineByPon[o.pon].length < 15) {
                  onlineByPon[o.pon].push(o.onuId);
                }
              });
              ...
              [1, 2, 3, 4].forEach((pon) => {
                const onuList = onlineByPon[pon];
                if (onuList && onuList.length > 0) {
                  setTimeout(() => {
                    if (isDone) return;
                    client.write(`interface epon ${pon}\r\n`);
                  }, delay);
                  delay += 250;

                  onuList.forEach((onuId) => {
                    setTimeout(() => {
                      if (isDone) return;
                      client.write(`show optical-rssi ${onuId}\r\n`);
                    }, delay);
                    delay += 120;
                  });
              ...
              setTimeout(() => {
                ...
                const blocks = buffer.split(/OLT\(config-epon-(\d+)\)#\s*show optical-rssi\s+(\d+)/i);
                for (let i = 1; i < blocks.length; i += 3) {
                  const pon = blocks[i];
                  const onuId = blocks[i + 1];
                  const text = blocks[i + 2] || '';

                  const key = `${pon}/${onuId}`;
                  const targetOnu = onusMap[key];

                  if (targetOnu) {
                    const txMatch = text.match(/Transmit Power\s*:\s*([+-]?\d+\.?\d*)/i);
                    const rxMatch = text.match(/Recieved Power\s*:\s*([+-]?\d+\.?\d*)/i);

                    if (txMatch) {
                      const formattedTx = `${parseFloat(txMatch[1]).toFixed(2)} dBm`;
                      targetOnu.tx_power = formattedTx;
                      targetOnu.tx = formattedTx;
                    }
                    if (rxMatch) {
                      const formattedRx = `${parseFloat(rxMatch[1]).toFixed(2)} dBm`;
                      targetOnu.rx_power = formattedRx;
                      targetOnu.rx = formattedRx;
                    }
                  }
                }
...
const fetchZimmlinkOnusViaTelnet = async (oltConfig, full = false) => {
  const cacheKey = oltConfig.id || oltConfig.host || 'default';
  const now = Date.now();
  const cached = zimmlinkCache[cacheKey];

  if (cached && cached.data && cached.data.length > 0 && (now - cached.time < 15000)) {
    logger.info(`[Zimmlink-Telnet] Serving ${cached.data.length} ONUs from instant cache for OLT ${cacheKey}`);
    return cached.data;
  }

  const freshData = await fetchZimmlinkOnusViaTelnetInternal(oltConfig, full);

  if (freshData && freshData.length > 0) {
    zimmlinkCache[cacheKey] = { time: Date.now(), data: freshData };
    return freshData;
  } else if (cached && cached.data && cached.data.length > 0) {
    logger.warn(`[Zimmlink-Telnet] Telnet returned empty/busy. Serving fallback cached ${cached.data.length} ONUs.`);
    return cached.data;
  }
...
async function getOltStatsInternal(id, full = false) {
  ...
  return new Promise((resolve) => {
    ...
    // OPTIMIZATION: Full data: 35s (for 100+ ONU optical power reading), Summary: 12s
    // If OLT offline, fail fast instead of hanging
    const timeoutMs = full ? 35000 : 12000;
    const globalTimeout = setTimeout(() => {
      if (stats.onus && stats.onus.length > 0) {
        logger.warn(`[OLT-SNMP] Timeout reached (${timeoutMs}ms) for ${olt.name}, returning ${stats.onus.length} partial/populated ONUs.`);
        delete stats.error;
      } else {
    ...
    (async () => {
      try {
        ...
        if (brandKey === 'zimmlink' || olt.brand === 'zimmlink') {
          activeProfile = selectedProfiles[0] || { name: 'ZIMMLINK_EPON_C', probe_oid: '1.3.6.1.4.1.25355.3.2.6.3.2.1.39', __brandKey: 'zimmlink' };
        } else {
        ...
        if (!activeProfile) {
          if (selectedProfiles && selectedProfiles.length > 0) {
            activeProfile = selectedProfiles[0];
            logger.info(`[OLT-SNMP] OID table probe returned empty for ${olt.name}, falling back to configured brand profile (${activeProfile.name})`);
          } else {
        ...
        // Instant Telnet fetch for Zimmlink (skip useless SNMP table walk timeout)
        if (detectedBrandKey === 'zimmlink' || olt.brand === 'zimmlink') {
          const telnetOnus = await fetchZimmlinkOnusViaTelnet(olt, full);
          if (telnetOnus && telnetOnus.length > 0) {
            stats.onus = telnetOnus;
            stats.onus_total = telnetOnus.length;
            stats.onus_online = telnetOnus.filter(o => o.status === 'Online').length;
            stats.onus_offline = telnetOnus.filter(o => o.status === 'Offline').length;
        ...
        if (Object.keys(statusMap).length === 0 && (olt.web_user || olt.telnet_port)) {
          const telnetOnus = await fetchZimmlinkOnusViaTelnet(olt, full);
          if (telnetOnus && telnetOnus.length > 0) {
            stats.onus = telnetOnus;
            stats.onus_total = telnetOnus.length;
        ...
        for (const idx of allIndices) {
          ...
          onusMapByIdx[idx] = onuItem;
          onusList.push(onuItem);
        }
        ...
        if (full) {
          try {
            const [cardRes, unauthRes, snRes, rxRes, txRes, distRes, fwRes, upRes, reasonRes] = await Promise.all([
              fetchCardMetrics(session, detectedBrandKey, stats).catch(() => null),
              fetchUnauthOnus(session, activeProfile, stats).catch(() => null),
              pickSnTable(session, activeProfile).then(res => res.map || {}).catch(() => ({})),
              activeProfile.rx_power_table ? slowWalk(session, activeProfile.rx_power_table).catch(() => ({})) : Promise.resolve({}),
              activeProfile.tx_power_table ? slowWalk(session, activeProfile.tx_power_table).catch(() => ({})) : Promise.resolve({}),
              activeProfile.distance_table ? slowWalk(session, activeProfile.distance_table).catch(() => ({})) : Promise.resolve({}),
              activeProfile.firmware_table ? slowWalk(session, activeProfile.firmware_table).catch(() => ({})) : Promise.resolve({}),
              activeProfile.uptime_table ? slowWalk(session, activeProfile.uptime_table).catch(() => ({})) : Promise.resolve({}),
              activeProfile.offline_reason_table ? slowWalk(session, activeProfile.offline_reason_table).catch(() => ({})) : Promise.resolve({})
            ]);
            ...
            for (const item of onusList) {
              ...
              if (activeProfile.distance_table && distInt != null && distInt > 0) {
                ...
                item.distance = tenths ? (distInt / 10).toFixed(1) + ' m' : `${distInt} m`;
              }

              if (fwVal) item.firmware = safeToString(fwVal) || '-';
              if (upVal) item.uptime = decodeUptime(bufferToInt(upVal)) || '-';

              if (item.status === 'Offline') {
                const rawReason = getByIdx(reasonMap, idx);
                item.offline_reason = translateOfflineReason(detectedBrandKey, rawReason) || null;
              }
            ...
            logger.warn(`[OLT-SNMP] Detail fetch parallel error for ${olt.name}: ${e.message}`);
          }
        }

        await systemMetricsPromise;

        stats.onus = onusList.sort((a, b) => a.name.localeCompare(b.name));
        safeResolve(stats);
      } catch (err) {
        stats.error = err.message;
        safeResolve(stats);
...
/**
 * Ambil daftar Port Uplink ZTE C320 & status VLAN-nya via Telnet CLI.
 */
async function getZteUplinkPorts(oltId) {
  const olt = getOltById(oltId);
  if (!olt) throw new Error('OLT tidak ditemukan');

  const defaultPorts = ['gei_1/3/1', 'gei_1/3/2', 'gei_1/3/3', 'gei_1/3/4'];
  const cmds = [
    'terminal length 0',
    ...defaultPorts.map(p => `show interface ${p}`),
    ...defaultPorts.map(p => `show vlan port ${p}`)
  ];

  const rawOutput = await telnetLoginAndRun(olt.host, olt.web_user, olt.web_password, cmds, telnetOptsFromOlt(olt));
  
  const portsList = [];
  defaultPorts.forEach(portName => {
    const safePort = portName.replace(/\//g, '\\/');
    const ifaceBlockRegex = new RegExp(`${safePort}\\s+is\\s+([^,\\n]+),\\s+line protocol is\\s+([^,\\n]+)`, 'i');
    const matchIface = rawOutput.match(ifaceBlockRegex);

    let adminStatus = 'down';
    let linkStatus = 'down';
    if (matchIface) {
      adminStatus = matchIface[1].trim().toLowerCase().includes('up') ? 'up' : 'down';
      linkStatus = matchIface[2].trim().toLowerCase().includes('up') ? 'up' : 'down';
    }

    const descRegex = new RegExp(`${safePort}[\\s\\S]*?Description is\\s+([^\\r\\n]+)`, 'i');
    const matchDesc = rawOutput.match(descRegex);
    const description = matchDesc ? matchDesc[1].trim() : 'none';
    ...
    const vlanIdx = rawOutput.indexOf(`show vlan port ${portName}`);
    if (vlanIdx !== -1) {
      const vlanSub = rawOutput.substring(vlanIdx, vlanIdx + 1000);
      const modeMatch = vlanSub.match(/PortMode\\s+Pvid[\\s\\S]*?\\n(\\w+)>?\\d*\\s+(\\d+)/i);
      if (modeMatch) {
        mode = modeMatch[1].trim();
        pvid = modeMatch[2].trim();
      }
      const taggedMatch = vlanSub.match(/TaggedVlan:\\s*\\n([0-9,\\s\\-]+)/i);
      if (taggedMatch && taggedMatch[1]) {
        taggedVlans = taggedMatch[1].trim().split(/[\\s,]+/).filter(Boolean);
      }
      const untaggedMatch = vlanSub.match(/UntaggedVlan:\\s*\\n([0-9,\\s\\-]+)/i);
      if (untaggedMatch && untaggedMatch[1]) {
        untaggedVlans = untaggedMatch[1].trim().split(/[\\s,]+/).filter(Boolean);
      }
    }
    ...
    portsList.push({
      ...
      mode,
      pvid,
      taggedVlans: taggedVlans.join(', ') || '-',
      untaggedVlans: untaggedVlans.join(', ') || '-'
    });
  });

  return { olt, ports: portsList };
}

/**
 * Konfigurasi Uplink Port & VLAN ZTE C320 via Telnet CLI.
 */
async function configZteUplinkPort(oltId, data) {
  const olt = getOltById(oltId);
  if (!olt) throw new Error('OLT tidak ditemukan');

  const { port, adminState, mode, pvid, addVlan, removeVlan, description } = data;
  if (!port) throw new Error('Nama port uplink harus diisi');
  ...
  if (description !== undefined && description !== null && String(description).trim() !== '') {
    cmds.push(`description ${description.trim()}`);
  }
  if (adminState === 'up') {
  ...
  if (mode) {
    cmds.push(`switchport mode ${mode}`);
  }

  if (pvid) {
    cmds.push(`switchport pvid ${pvid}`);
  }

  if (addVlan) {
    const vlans = String(addVlan).split(/[\\s,]+/).filter(Boolean);
    vlans.forEach(v => {
      cmds.push(`switchport vlan ${v} tag`);
    });
  }

  if (removeVlan) {
    const vlans = String(removeVlan).split(/[\\s,]+/).filter(Boolean);
    vlans.forEach(v => {
      cmds.push(`no switchport vlan ${v}`);
    });
  }
...
module.exports = {
  ...
  configureOnuWan, configureZteWanViaGoApi, configureWanViaAcs, getZteUplinkPorts, configZteUplinkPort
};

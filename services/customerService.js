...
function getAllCustomers(search = '', routerId = null, filterStatus = '') {
  // Gunakan fallback ke new Date() jika getCurrentDateInTimezone() menghasilkan Date invalid
  let now;
  try {
    now = getCurrentDateInTimezone();
    if (isNaN(now.getTime())) throw new Error('Invalid date from getCurrentDateInTimezone');
  } catch (e) {
    now = new Date();
  }
  ...
  // Pastikan month & year valid (bukan NaN) sebelum dimasukkan ke query
  ...
  const base = `
    ...
    LEFT JOIN customer_usage u ON u.customer_id = c.id AND u.period_month = ${safeMonth} AND u.period_year = ${safeYear}
  `;

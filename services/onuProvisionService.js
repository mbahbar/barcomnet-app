...
class ONUProvisionService {
  ...
  /**
   * Connect ke OLT via SSH atau Telnet (apabila port 23)
   */
  async connectSSH(oltConfig) {
    const port = Number(oltConfig.port) || 22;
    if (port === 23) {
      return { isTelnet: true, oltConfig, end: () => {} };
    }
  ...
  /**
   * Helper Telnet execution apabila koneksi menggunakan Port 23
   */
  async executeTelnetCommands(oltConfig, commands) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeoutTimer = setTimeout(() => {
        socket.destroy();
        reject(new Error('Telnet command sequence timeout'));
      }, 45000);

      socket.connect(Number(oltConfig.port) || 23, oltConfig.host, () => {
        logger.info(`[ONU-Provision Telnet] Connected to ${oltConfig.host}:${oltConfig.port || 23}`);
      });

      const writeNext = () => {
        if (cmdIndex < cmdList.length) {
          const cmd = sanitizeCliInput(cmdList[cmdIndex++]);
          logger.info(`[ONU-Provision Telnet] Command sent: ${cmd}`);
          socket.write(cmd + '\r\n');
        } else {
          setTimeout(() => {
            socket.end();
      ...
      const loginRe = /(login|username|user name)\s*[:>]\s*$/im;
      ...
      socket.on('data', (data) => {
        ...
        if (buffer.includes('--More--') || buffer.includes('--Press Enter--')) {
          socket.write(' ');
          buffer = buffer.replace(/--More--|--Press Enter--/g, '');
        }

        if (!loggedIn) {
          if (loginRe.test(buffer)) {
            socket.write((oltConfig.username || 'zte') + '\r\n');
            buffer = '';
          } else if (passRe.test(buffer)) {
            socket.write((oltConfig.password || 'zte') + '\r\n');
            buffer = '';
          } else if (promptRe.test(buffer)) {
            loggedIn = true;
            buffer = '';
  ...
  /**
   * Execute multiple commands sequentially on a single SSH or Telnet stream
   */
  async executeCommands(conn, commands, promptPattern = /#|>/) {
    if (conn && conn.isTelnet) {
      return this.executeTelnetCommands(conn.oltConfig, commands);
    }
    return new Promise((resolve, reject) => {
  ...
  async fullProvision(oltConfig, mikrotikConfig, params) {
    ...
    try {
      ...
      } else if (vendor === 'HSGQ' || vendor === 'Zimmlink' || vendor === 'ZIMMLINK') {

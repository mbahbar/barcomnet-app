  ...
  try {
    ...
    if (!hasWlan || !hasWan || !hasRx) {
      ...
      if (isTr181) {
        ...
        informPvs.push(['Device.ManagementServer.PeriodicInformInterval', '120', 'xsd:unsignedInt']);
      } else {
        informPvs.push(['InternetGatewayDevice.ManagementServer.PeriodicInformEnable', 'true', 'xsd:boolean']);
        informPvs.push(['InternetGatewayDevice.ManagementServer.PeriodicInformInterval', '120', 'xsd:unsignedInt']);
      }
      db.prepare(
        `INSERT INTO acs_tasks (device_id, name, payload, status, created_at, updated_at)
  ...
  try {
    ...
    return new Promise((resolve) => {
      const executeRequest = (authHeader = null) => {
        ...
        req.on('error', (err) => {
          logger.debug(`[ACS] Connection request to ${deviceId} unreachable: ${err.message}`);
          resolve({ success: false, message: `Connection request failed: ${err.message}` });
        });

        req.on('timeout', () => {
          req.destroy();
          logger.debug(`[ACS] Connection request to ${deviceId} timed out`);
...
function createBuiltinTask(deviceId, taskName, payload) {
  try {
    ...
    // Cancel any existing pending task with the same name for this device to prevent queue stacking
    db.prepare(`UPDATE acs_tasks SET status = 'cancelled', updated_at = ? WHERE device_id = ? AND name = ? AND status = 'pending'`).run(now, deviceId, taskName);

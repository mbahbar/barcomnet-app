      ...
      for (const r of routers) {
        try {
          ...
          for (const s of actives) {
            ...
            // Validasi customer ID - skip jika invalid
            if (!cust.id || isNaN(cust.id)) {
              logger.warn(`[CRON] Skipping usage tracking: invalid customer ID for ${username}`);
              continue;
            }

            // Pastikan bytes-in/bytes-out ada dan valid
            const rawIn = s['bytes-in'];
            const rawOut = s['bytes-out'];
            if (rawIn === undefined && rawOut === undefined) continue;

            const totalIn = parseInt(rawIn) || 0;
            const totalOut = parseInt(rawOut) || 0;
            ...
            // Final NaN guard sebelum masuk ke database
            if (isNaN(deltaIn) || isNaN(deltaOut) || isNaN(totalIn) || isNaN(totalOut)) {
              logger.warn(`[CRON] Skipping usage update for ${username}: invalid byte values (in=${rawIn}, out=${rawOut})`);
              continue;
            }
